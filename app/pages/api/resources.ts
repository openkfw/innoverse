...
import { GetResourcesRequest, GetResourcesResponse } from '../../entities/resource';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<RequestError | GetResourcesResponse>) {
  try {
    if (req.method === 'GET') {
      ...
      if (!parseInt(limit)) throw new Error(`Request parameter Limit is invalid: ${limit}`);

      if (repo !== 'db' && repo !== 'memory')
        throw new Error(`Request parameter Repo ("db" or "memory") is invalid: ${repo}`);

      // Query the repository and transform the response data into the right format
      if (repo === 'memory') {
        const response: GetResourcesResponse = {
          resources: getResourcesFromMemory(memoryClient, parseInt(limit)).map((r) => ({
            id: r.id,
            text: r.text,
          })),
        };
